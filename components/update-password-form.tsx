...
export function UpdatePasswordForm({ flow = "recovery" }: { flow?: "recovery" | "invite" }) {
  ...
  return (
    <div>
      <span className="grid size-12 place-items-center rounded-xl bg-pink-50 text-pink-600"><KeyRound aria-hidden="true" /></span>
      <h1 className="mt-5 text-3xl font-semibold tracking-tight text-slate-950">
        {flow === "invite" ? "Finish setting up your account" : "Choose a new password"}
      </h1>
      <p className="mt-2 text-sm leading-6 text-slate-600">
        {flow === "invite"
          ? `Create a password with at least ${PASSWORD_MIN_LENGTH} characters to accept your invitation.`
          : `Make it memorable and at least ${PASSWORD_MIN_LENGTH} characters long.`}
      </p>
      {state.success ? (
        <div className="mt-8 space-y-5">
          <FormMessage success={state.success} />
          <Button asChild className="h-11 w-full bg-pink-600 hover:bg-pink-500"><Link href="/dashboard">Continue to dashboard</Link></Button>
        </div>
      ) : (
        <form action={formAction} className="mt-8 space-y-5" onSubmit={validateBeforeSubmit}>
          ...
          <Button className="h-11 w-full bg-pink-600 hover:bg-pink-500" disabled={pending} type="submit">
            ...
            {pending ? "Saving password…" : flow === "invite" ? "Complete account setup" : "Update password"}
          </Button>
        </form>
      )}
    </div>
