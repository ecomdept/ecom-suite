    ...
    <details className="mt-8 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm">
      ...
      <div className={`grid border-t border-stone-100 ${canManageAccess ? "lg:grid-cols-2 lg:divide-x lg:divide-slate-100" : ""}`}>
        <form action={settingsAction} className="grid content-start gap-5 p-5 sm:p-7">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {projectType === "retainer" && <div className="space-y-2"><Label htmlFor="settings-retainer">Hours per two-week sprint</Label><Input defaultValue={retainerHours ?? ""} id="settings-retainer" min="0" name="retainerHours" step="0.25" type="number" /></div>}
          ...
          {projectType === "retainer" && <div className="rounded-xl border border-stone-200 p-4"><label className="flex items-start gap-3"><input className="mt-0.5 size-4 rounded border-slate-300 text-pink-600 focus:ring-pink-600" defaultChecked={rolloverEnabled} name="rolloverEnabled" type="checkbox" /><span><span className="block text-sm font-medium">Carry unused hours into the next sprint</span><span className="mt-1 block text-xs text-slate-500">Rollover is reported separately from newly renewed capacity.</span></span></label><div className="mt-4 space-y-2"><Label htmlFor="settings-rollover-cap">Rollover cap in hours <span className="font-normal text-slate-400">(optional)</span></Label><Input defaultValue={rolloverCapHours ?? ""} id="settings-rollover-cap" min="0" name="rolloverCapHours" placeholder="No cap" step="0.25" type="number" /></div></div>}
