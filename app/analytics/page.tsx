...
import { SprintTrendCharts } from "@/components/analytics/sprint-trend-charts";
import { TeamCostRates } from "@/components/analytics/team-cost-rates";
import { requireUser } from "@/lib/auth/session";
import { ROLE_LABELS, isAppRole } from "@/lib/auth/roles";
import { formatSprintDate, getSprintWindow } from "@/lib/projects/sprints";
...
const shift = (value: string, days: number) => iso(new Date(new Date(`${value}T00:00:00.000Z`).getTime() + days * DAY));
...
export default async function AnalyticsPage() {
  ...
  const [{ data: projects }, { data: costRates }] = await Promise.all([
    supabase.from("projects").select("id, name, project_type, retainer_hours, hourly_rate, sprint_start_date, status, risk, rollover_enabled, rollover_cap_hours").order("name"),
    supabase.from("team_cost_rates").select("user_id, hourly_cost"),
  ]);
  const projectRows = (projects ?? []) as Array<{ id: string; name: string; project_type: "retainer" | "new_build"; retainer_hours: number | null; hourly_rate: number | null; sprint_start_date: string; status: "active" | "on_hold" | "completed"; risk: "on_track" | "at_risk" | "off_track"; rollover_enabled: boolean; rollover_cap_hours: number | null }>;
  const agencyAnchor = projectRows.map((project) => project.sprint_start_date).sort()[0] ?? iso(new Date());
  const currentAgencySprint = getSprintWindow(agencyAnchor);
  const trendWindows = Array.from({ length: 6 }, (_, reverseIndex) => {
    const offset = (reverseIndex - 5) * 14;
    const start = shift(currentAgencySprint.start, offset);
    const endExclusive = shift(start, 14);
    return { start, endExclusive, label: new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", timeZone: "UTC" }).format(new Date(`${start}T00:00:00.000Z`)) };
  });
  ...
  const projectMetrics = projectRows.map((project) => {
    const sprint = getSprintWindow(project.sprint_start_date);
    const previousStart = shift(sprint.start, -14);
    const currentEntries = entries.filter((entry) => entry.project_id === project.id && entry.date >= sprint.start && entry.date < sprint.endExclusive);
    const previousHours = entries.filter((entry) => entry.project_id === project.id && entry.date >= previousStart && entry.date < sprint.start).reduce((total, entry) => total + entry.hours, 0);
    ...
    return { ...project, sprint, usedHours, rolloverHours, availableHours: Math.max(0, baseHours + rolloverHours - usedHours), revenue, cost, profit: revenue - cost };
  });
  ...
  const trends = trendWindows.map((window) => {
    ...
    projectRows.forEach((project) => {
      if (project.sprint_start_date >= window.endExclusive) return;
    ...
    return { label: window.label, revenue, cost, profit: revenue - cost, usedHours, unusedHours: unused, rolloverHours: rollover };
  });
  ...
  return (
    <main className="min-h-svh bg-[#f6f3ee] text-slate-950">
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-10 sm:px-8 sm:py-14">
        ...
        <section aria-label="Current sprint financial summary" className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="mt-8"><SprintTrendCharts trends={trends} /></div>
        ...
        <section className="mt-8 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm" aria-labelledby="portfolio-profitability-heading">
          <div className="flex flex-wrap items-end justify-between gap-4 px-5 py-5 sm:px-7"><div><h2 className="font-semibold" id="portfolio-profitability-heading">Current-cycle profitability</h2><p className="mt-1 text-sm text-slate-500">Commercial health by active engagement.</p></div><div className="flex gap-4 text-xs text-slate-500"><span>{unusedHours.toFixed(1)}h unused</span><span>{rolloverHours.toFixed(1)}h eligible rollover</span></div></div>
        ...
        <p className="mt-5 text-xs leading-5 text-slate-500">Profit is an operational estimate: retainer revenue is recognized from committed sprint hours at the project hourly rate; new-build revenue is recognized from logged hours. Labor cost uses the internal rates above. Overhead, taxes, discounts, invoices, and external expenses are not included.</p>
      </div>
    </main>
  );
}
