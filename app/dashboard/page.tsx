...
import { ClientSprintReport } from "@/components/dashboard/client-sprint-report";
...
import { getSprintWindow } from "@/lib/projects/sprints";
...
export default async function DashboardPage() {
  ...
  const sprintByProject = new Map((projects ?? []).map((project) => [project.id, getSprintWindow(project.sprint_start_date)]));
  const sprintStarts = [...sprintByProject.values()].map((sprint) => sprint.start).sort();
  const sprintEnds = [...sprintByProject.values()].map((sprint) => sprint.endExclusive).sort();
  const [{ data: tickets }, { data: timeEntries }, { data: committedHoursResult }] = projectIds.length
    ? await Promise.all([
        supabase.from("tickets").select("id, project_id, title, description, status, priority, ticket_type, due_date, assignee_id, estimated_hours, logged_hours, billable_amount, created_at").in("project_id", projectIds).order("created_at", { ascending: false }),
        isClient ? supabase.rpc("get_project_daily_time_totals", { period_start: new Date(new Date(`${sprintStarts[0]}T00:00:00.000Z`).getTime() - 70 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10), period_end: sprintEnds.at(-1) }) : Promise.resolve({ data: [] as Array<{ project_id: string; hours: number; work_date: string }> }),
  ...
  const attentionTickets = [...overdueTickets, ...unassignedTickets.filter((ticket) => !overdueTickets.some((overdue) => overdue.id === ticket.id))]
    .slice(0, 6);
  const sprintHours = new Map<string, number>();
  const previousSprintHours = new Map<string, number>();
  ...
  dailyTimeEntries.forEach((entry) => {
    const sprint = sprintByProject.get(entry.project_id);
    const entryDate = entry.work_date;
    if (sprint && entryDate >= sprint.start && entryDate < sprint.endExclusive) sprintHours.set(entry.project_id, (sprintHours.get(entry.project_id) ?? 0) + Number(entry.hours));
    else if (sprint) {
      const previousStart = new Date(new Date(`${sprint.start}T00:00:00.000Z`).getTime() - 14 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
      if (entryDate >= previousStart && entryDate < sprint.start) previousSprintHours.set(entry.project_id, (previousSprintHours.get(entry.project_id) ?? 0) + Number(entry.hours));
    }
  });
  ...
  const totalRetainer = retainers.reduce((total, project) => {
    ...
    const previousUnused = Math.max(0, base - Math.max(0, previousSprintHours.get(project.id) ?? 0));
  ...
  const totalSprintLogged = retainers.reduce((total, project) => total + Math.max(0, sprintHours.get(project.id) ?? 0), 0);
  const committedHours = Math.max(0, Number(committedHoursResult ?? 0));
  const remainingRetainer = Math.max(0, totalRetainer - totalSprintLogged - committedHours);
  const clientSprintPoints = clientProject ? Array.from({ length: 6 }, (_, index) => {
    const current = getSprintWindow(clientProject.sprint_start_date);
    const offset = (5 - index) * 14;
    const startDate = new Date(new Date(`${current.start}T00:00:00.000Z`).getTime() - offset * 86400000);
    const endDate = new Date(startDate.getTime() + 14 * 86400000);
    const start = startDate.toISOString().slice(0, 10);
    const end = endDate.toISOString().slice(0, 10);
    return { label: formatDate(start), used: dailyTimeEntries.filter((entry) => entry.project_id === clientProject.id && entry.work_date >= start && entry.work_date < end).reduce((sum, entry) => sum + Number(entry.hours), 0), capacity: Number(clientProject.retainer_hours ?? 0) };
  }) : [];
  const clientBoardTickets = (tickets ?? []).filter((ticket) => ticket.project_id === clientProject?.id && isTicketStatus(ticket.status) && isTicketPriority(ticket.priority) && isTicketType(ticket.ticket_type)).map((ticket) => ({ ...ticket, status: ticket.status as NonNullable<typeof ticket.status>, priority: ticket.priority as NonNullable<typeof ticket.priority>, ticket_type: ticket.ticket_type as NonNullable<typeof ticket.ticket_type>, estimated_hours: Number(ticket.estimated_hours ?? 0), logged_hours: Number(ticket.logged_hours ?? 0) }));
        ...
        {isClient ? (
          <>
            ...
            {clientProject ? <>
              ...
              <div className="mt-8"><ClientSprintReport points={clientSprintPoints}/></div>
